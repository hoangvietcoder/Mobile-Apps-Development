...
export const Create = ({ navigation, route }) => {
  ...
  const [color, setColor] = useState('#033366')
  ...
  return <View style={styles.container}>
    ...
    <TouchableOpacity style={[styles.saveButton, { backgroundColor:route.params.color ? route.params.color : Colors.primary}]} onPress={() => {
      if(title.length > 1) {
        route.params.saveCreate({title, color})
        navigation.dispatch(CommonActions.goBack())
      } else {
        setValidity(false)
      }
    }}>
      <Text style={{color: 'white', fontSize: 24, fontWeight: 'bold'}}>Create</Text>
    </TouchableOpacity>
  </View>
}
